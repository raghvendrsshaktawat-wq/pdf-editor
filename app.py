...
def draw_text_box(
    ...
    *,
    ...
    """
    Adjustable text box:
    - box_width       : width of box (px)
    - box_height_mult : height = fontsize * box_height_mult
    - border_width    : border thickness (pt)
    - offset_x / y    : shift start position from anchor baseline
    """
    if not text:
        return
    ...
    box_height = fontsize * box_height_mult
    top_edge = y - fontsize * 1.3
...
def update_pdf(pdf_bytes, entries, surveyor_name=None):
    ...
    if surveyor_name:
        for page in doc:
            ...
            rect = fitz.Rect(text_x - 2, text_y - 14 * 1.3, text_x + 150, text_y + 5)
            page.draw_rect(rect, color=(1, 1, 1), fill=(1, 1, 1), width=0)
    ...
    if not aperture_positions and len(entries) > 0:
        ...
        for idx in range(len(entries)):
            aperture_positions.append(
                (0, fitz.Rect(fallback_x, fallback_y + (idx * 40),
                              fallback_x + 250, fallback_y + (idx * 40) + 20))
            )
...
st.divider()
st.caption("Red border/text in PDF indicates >75 mm difference from order sizes.")
