# ==========================
# Fenesta WCS Survey Editor v3.0
# ==========================
...
.fen-header { background: linear-gradient(135deg, #1A1A2E 0%, #16213E 100%); padding: 18px 32px; border-radius: 16px; display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; box-shadow: 0 4px 20px rgba(26,26,46,0.18); }
...
.metric-card { background: white; border-radius: 12px; padding: 14px 20px; flex: 1; min-width: 130px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border-left: 4px solid #F47920; }
.metric-card.ok { border-left-color: #22c55e; }
.metric-card.warn { border-left-color: #f59e0b; }
.metric-card.danger { border-left-color: #ef4444; }
.mk { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #94a3b8; }
.mv { font-size: 26px; font-weight: 800; color: #1A1A2E; margin-top: 2px; }
...
.legend { display: flex; gap: 16px; align-items: center; background: white; border-radius: 10px; padding: 10px 16px; box-shadow: 0 1px 4px rgba(0,0,0,0.06); margin-bottom: 16px; flex-wrap: wrap; }
...
[data-testid="stFileUploader"] { background: white !important; border-radius: 14px !important; padding: 8px !important; box-shadow: 0 2px 8px rgba(0,0,0,0.06) !important; }
.stDownloadButton > button, .stButton > button { background: linear-gradient(135deg, #F47920, #e8671a) !important; color: white !important; border: none !important; border-radius: 10px !important; font-weight: 700 !important; box-shadow: 0 2px 8px rgba(244,121,32,0.3) !important; }
[data-testid="stExpander"] { background: white !important; border-radius: 14px !important; border: 1px solid #e2e8f0 !important; box-shadow: 0 2px 8px rgba(0,0,0,0.05) !important; margin-bottom: 14px !important; }
</style>
...
def parse_wcs_pdf(file_bytes):
    ...
    # flatten to single line for easier regex
    flat = " ".join(text.split())
    return parse_wcs_flat(flat)

def parse_wcs_flat(text):
    meta = {}
    m = re.search(r'\b(W\d{7,})\b', text);          meta['order_no']  = m.group(1) if m else ''
    m = re.search(r'MSC No\.?\s*([\d]+)', text);     meta['msc_no']    = m.group(1) if m else ''
    m = re.search(r'Zone\s+([A-Z]{4,})', text);      meta['zone']      = m.group(1) if m else ''
    m = re.search(r'Tel\.\s+\d[\d\s]+Email\s+\S+\s+To\s+Deliver to\s+[\w\s,]+?(?:INDIA)\s+([A-Z][A-Z ]+?)\s+[A-Z]{4,}', text)
    meta['customer'] = m.group(1).strip() if m else ''

    # Split on "Aperture Size" — each chunk ENDS with one line item
    chunks = re.split(r'Aperture Size', text)

    rows = []
    for i, chunk in enumerate(chunks):
        tail = chunk[-500:].strip()

        # Try match with inline description: 0NNN 1 <desc> WIDTH HEIGHT GLAZING
        m1 = re.search(
            r'\b(0\d{3})\s+1\s+((?:[A-Za-z][A-Za-z0-9 \.\-\/]+?)\s+)?(\d{3,4})\s+(\d{3,4})\s+((?:SG|DG|TG|Louver)[\w\s_]+?)\s*$',
            tail
        )
        # Plain: 0NNN 1 WIDTH HEIGHT GLAZING (no description inline)
        m2 = re.search(
            r'\b(0\d{3})\s+1\s+(\d{3,4})\s+(\d{3,4})\s+((?:SG|DG|TG|Louver)[\w\s_]+?)\s*$',
            tail
        )
        ...
        if m2 and (not m1 or len(m1.group(0)) <= len(m2.group(0)) + 2):
            sales_line   = m2.group(1)
            order_width  = int(m2.group(2))
            order_height = int(m2.group(3))
            # Description: keyword just before the sales line number
            before = tail[:m2.start()].strip()
            dm = re.search(r'((?:Casement|Bay|Fixed|Sliding|Door|Ventilator|Louvre)[\w\s\.\-\/]*)\s*$', before, re.I)
            description = dm.group(1).strip() if dm else ''
        else:
            sales_line   = m1.group(1)
            description  = (m1.group(2) or '').strip()
            order_width  = int(m1.group(3))
            order_height = int(m1.group(4))

        # Ref / Loc / System — appear in the NEXT chunk before "Reference Location System"
        ref = loc = sys = ''
        if i + 1 < len(chunks):
            nc = chunks[i + 1][:600]
            rm = re.search(
                r'\b([A-Z]{1,3}\d+[A-Z0-9\/]*)\s+(\d[A-Z]\/\d+)\s+(SY\d+[\w\s\(\)]+?)\s*(?:Reference|Colour)',
                nc
            )
            if rm:
                ref = rm.group(1)
                loc = rm.group(2)
                sys = rm.group(3).strip()
...
for i, pdf_file in enumerate(uploaded_pdfs, 1):
    ...
    with st.expander(f"📄  {pdf_file.name}", expanded=(i == 1)):
        ...
        if not rows:
            st.error("⚠ No sales line items detected. Check if this is a text-based Fenesta WCS PDF.")
            continue
        ...
        n_ok, n_warn, n_danger, n_empty = tols.count('ok'), tols.count('warn'), tols.count('danger'), tols.count('empty')
        n_total = len(tols)
        ...
        st.download_button(
            f"⬇ Download {out_name}",
            data=overlaid, file_name=out_name, mime="application/pdf",
            key=f"dl_{i}"
        )
...
if all_file_data:
    ...
    st.download_button(
        ...
        data=excel_buf.getvalue(), file_name=xl_name,
